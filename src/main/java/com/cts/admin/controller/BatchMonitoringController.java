...
public class BatchMonitoringController extends GenericForwardComposer<Component> {
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		tabBatchCapture.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
			@Override
		...
		tabInward.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
			@Override
			public void onEvent(Event e) throws Exception {
		...
		tabOutward.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
			@Override
			public void onEvent(Event e) throws Exception {
		...
		batchSearchButton.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
			@Override
			public void onEvent(Event e) throws Exception {
				refreshActiveTab();
			}
		...
		batchSearchTextbox.addEventListener(Events.ON_OK, new EventListener<Event>() {
			@Override
			public void onEvent(Event e) throws Exception {
				refreshActiveTab();
			}
	...
	private void switchTab(Tab tab) {
		...
		tabBatchCapture.setSclass(tab == Tab.BATCH_CAPTURE ? "batch-tab-btn batch-tab-active" : "batch-tab-btn");
		tabInward.setSclass(tab == Tab.INWARD ? "batch-tab-btn batch-tab-active" : "batch-tab-btn");
		tabOutward.setSclass(tab == Tab.OUTWARD ? "batch-tab-btn batch-tab-active" : "batch-tab-btn");
	...
	private void loadBatchCapture() {
		...
		try {
			...
			rebuildListhead("Batch ID", "10%", "Branch", "15%", "Cheque Count", "15%", "Created By", "25%",
					"Created At", "35%");
	...
	private void loadInward() {
		...
		try {
			...
			rebuildListhead("Batch ID", "8%", "Cheque Count", "9%", "Status", "14%", "Maker", "12%", "Checker", "12%",
					"Maker Receive Time", "15%", "Checker Receive Time", "15%", "Batch Completion Time", "15%");
	...
	private List<Batch> filterCapture(List<Batch> all) {
		...
		for (Batch b : all) {
			boolean match = String.valueOf(b.getBatchId()).contains(lower)
	...
	private Listcell buildStatusCell(String status) {
		...
		} else if (upper.contains("PROGRESS") || upper.contains("PENDING") || upper.contains("PROCESSING")
				|| upper.contains("MAKER") || upper.contains("CHECKER") || upper.equals("START")) {
