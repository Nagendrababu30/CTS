...
import com.cts.inward.dao.ReportDao;
import com.cts.inward.dao.ReportDaoImpl;
...
public class ReportController extends GenericForwardComposer<Component> {
    ...
    private ReportDao reportDao = ReportDaoImpl.of();
    ...
    private void loadReportData() {
        try {
            ...
            List<NpciBatchData> sentBatches = reportDao.getSentToCheckerBatches();

            for (NpciBatchData batch : sentBatches) {
                Row row = new Row();
                ...
                actionBtn.addEventListener(Events.ON_CLICK, e -> {
                    try {
                        // 1. Fetch exact report counts (MICR and Data Entry repairs) from DB
                        Map<String, Object> reportParams = reportDao.getBatchReportDetails(batch.getBatchId());

                        // 2. Load Jasper template from WEB-INF/reports/batch_summary.jrxml
                        InputStream jrxmlStream = Sessions.getCurrent().getWebApp()
                                                  .getResourceAsStream("/WEB-INF/reports/batch_summary.jrxml");
