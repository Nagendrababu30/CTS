...
public class RoleController extends GenericForwardComposer<Component> {
    ...
    private void loadRoles() {
        ...
        try {
            ...
            for (Role role : roles) {
                ...
                toggleBtn.setSclass(isActive 
                    ? "role-action-button role-deactivate-button"
    ...
    private void saveRole(String roleName, String description, Window window) {
        ...
        if (roleName == null || roleName.trim().isEmpty()) {
            Messagebox.show("Please enter a role name.", "Validation",
        ...
        try {
            ...
            if (roleService.createRole(newRole)) {
                ...
                Messagebox.show("Failed to create role.", "Error",
                        Messagebox.OK, Messagebox.ERROR);
            }
        } catch (Exception e) {
            e.printStackTrace();
            Messagebox.show("Error creating role: " + e.getMessage(), "Error",
                    Messagebox.OK, Messagebox.ERROR);
        }
    }
    ...
    private void toggleRoleStatus(Long roleId, boolean currentlyActive) {
        ...
        try {
            ...
            if (roleService.updateRoleStatus(roleId, newStatus)) {
                Messagebox.show("Role status updated to " + newStatus + ".", "Success",
                        Messagebox.OK, Messagebox.INFORMATION);
                loadRoles(); // Reload the role list
            } else {
                Messagebox.show("Failed to update role status.", "Error",
                        Messagebox.OK, Messagebox.ERROR);
            }
        } catch (Exception e) {
            e.printStackTrace();
            Messagebox.show("Error updating role status: " + e.getMessage(), "Error",
                    Messagebox.OK, Messagebox.ERROR);
        }
    }
