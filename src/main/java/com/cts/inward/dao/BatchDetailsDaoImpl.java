...
public class BatchDetailsDaoImpl implements BatchDetailsDao {
	...
	public static BatchDetailsDaoImpl of() {
		return new BatchDetailsDaoImpl(ConnectionPool.getDataSource());
	}
		...
		try (Connection connection = dataSource.getConnection();
			...
			try (ResultSet resultSet = statement.executeQuery()) {
				...
				if (resultSet.next()) {
					...
					micrDetails.put("chequeNumber", resultSet.getString("cheque_number"));

					micrDetails.put("currentMicr", resultSet.getString("current_micr"));

					micrDetails.put("oldMicr", resultSet.getString("old_micr"));

					micrDetails.put("correctedMicr", resultSet.getString("corrected_micr"));
				}
			}

		} catch (Exception e) {

			throw new RuntimeException("Error retrieving MICR details for cheque: " + chequeNumber, e);
		}
	...
	@Override
	public List<Map<String, Object>> getChequesByBatchId(Long batchId) {

		if (batchId == null) {
			return new java.util.ArrayList<>();
		}

		// Check if this batch is a Re-Verify batch (has cheques returned to maker by
		// checker)
		...
		String sql;
		if (isReverifyBatch) {
			// In Re-Verify batches, ONLY load cheques that were returned to maker by
			// checker
			sql = """
					SELECT
					    c.cheque_number,
		...
		try (Connection connection = dataSource.getConnection();
			...
			try (ResultSet resultSet = statement.executeQuery()) {
				...
				while (resultSet.next()) {
					...
					cheque.put("chequeNumber", resultSet.getString("cheque_number"));

					cheque.put("batchId", resultSet.getLong("batch_id"));

					cheque.put("accountNumber", resultSet.getString("account_number"));

					/*
					 * Database column is payee_name. Existing Java Map key remains drawerName so
					 * existing controller/ZUL does not break.
					 */
					cheque.put("drawerName", resultSet.getString("payee_name"));

					cheque.put("amount", resultSet.getBigDecimal("amount"));

					cheque.put("micrCode", resultSet.getString("micr_code"));

					cheque.put("chequeDate", resultSet.getDate("cheque_date"));

					cheque.put("status", resultSet.getString("cheque_status"));

					cheque.put("cheque_status", resultSet.getString("cheque_status"));

					cheque.put("checker_action", resultSet.getString("checker_action"));

					cheque.put("checkerAction", resultSet.getString("checker_action"));

					cheque.put("checker_id", resultSet.getObject("checker_id"));

					cheque.put("checkerId", resultSet.getObject("checker_id"));

					cheque.put("returnReasonCode", resultSet.getString("return_reason_code"));

					cheque.put("returnReasonDescription", resultSet.getString("return_reason_description"));

					cheque.put("makerRemarks", resultSet.getString("maker_remarks"));

					cheque.put("ocrMicrCode", resultSet.getString("ocr_micr_code"));
			...
			throw new RuntimeException("Error retrieving cheques for batch: " + batchId, e);
		}
	...
	public Map<String, Object> getDataEntryDetails(String chequeNumber) {
		...
		try (Connection connection = dataSource.getConnection()) {
			...
			try (PreparedStatement statement = connection.prepareStatement(chequeSql)) {
				...
				try (ResultSet rs = statement.executeQuery()) {
					...
					if (rs.next()) {
						...
						details.put("chequeNumber", rs.getString("cheque_number"));

						details.put("oldChequeNumber", rs.getString("cheque_number"));

						details.put("oldAccountNumber", rs.getString("account_number"));

						details.put("oldAmount", rs.getString("amount"));
				...
				try (ResultSet rs = statement.executeQuery()) {
					...
					while (rs.next()) {
						...
						if ("ACCOUNT_NUMBER".equals(fieldName)) {
							...
							details.put("newAccountNumber", newValue);

						} else if ("AMOUNT".equals(fieldName)) {

							details.put("newAmount", newValue);

						} else if ("CHEQUE_DATE".equals(fieldName)) {

							details.put("newChequeDate", newValue);

						} else if ("CHEQUE_NUMBER".equals(fieldName)) {

							details.put("newChequeNumber", newValue);
						}
					}
				}
			}
			...
			throw new RuntimeException("Error retrieving Data Entry details " + "for cheque: " + chequeNumber, e);
		}
		...
		try (Connection connection = dataSource.getConnection();
			...
			try (ResultSet resultSet = statement.executeQuery()) {
				...
				if (resultSet.next()) {
					...
					cbsDetails.put("chequeNumber", resultSet.getString("cheque_number"));

					cbsDetails.put("accountNumber", resultSet.getString("account_number"));

					cbsDetails.put("chequeDate", resultSet.getDate("cheque_date"));

					cbsDetails.put("chequeAmount", resultSet.getBigDecimal("cheque_amount"));

					cbsDetails.put("accountStatus", resultSet.getString("account_status"));

					cbsDetails.put("availableBalance", resultSet.getBigDecimal("available_balance"));
			...
			throw new RuntimeException("Error retrieving CBS validation details " + "for cheque: " + chequeNumber, e);
		}
	...
	@Override
	public void saveCheckerDecision(String chequeNumber, String status, List<String> rejectionReasonCodes,
			List<String> returnReasonCodes, Integer checkerId, String checkerAction, String remarks) {

		String primaryRejectionCode = (rejectionReasonCodes != null && !rejectionReasonCodes.isEmpty())
				? rejectionReasonCodes.get(0)
				: null;
		String primaryReturnCode = (returnReasonCodes != null && !returnReasonCodes.isEmpty())
				? returnReasonCodes.get(0)
				: null;
		...
		try (Connection connection = dataSource.getConnection()) {
			...
			try {
				...
				try (PreparedStatement ps = connection.prepareStatement(updateSql)) {
					...
					if (primaryRejectionCode != null)
						ps.setString(2, primaryRejectionCode);
					else
						ps.setNull(2, java.sql.Types.VARCHAR);
					if (primaryReturnCode != null)
						ps.setString(3, primaryReturnCode);
					else
						ps.setNull(3, java.sql.Types.VARCHAR);
					if (checkerId != null)
						ps.setInt(4, checkerId);
					else
						ps.setNull(4, java.sql.Types.INTEGER);
					if (checkerAction != null)
						ps.setString(5, checkerAction);
					else
						ps.setNull(5, java.sql.Types.VARCHAR);
					if (finalRemarks != null)
						ps.setString(6, finalRemarks);
					else
				...
				if (updatedRows == 0) {
					try (PreparedStatement insertPs = connection.prepareStatement(insertSql)) {
						...
						if (primaryRejectionCode != null)
							insertPs.setString(3, primaryRejectionCode);
						else
							insertPs.setNull(3, java.sql.Types.VARCHAR);
						if (primaryReturnCode != null)
							insertPs.setString(4, primaryReturnCode);
						else
							insertPs.setNull(4, java.sql.Types.VARCHAR);
						if (checkerId != null)
							insertPs.setInt(5, checkerId);
						else
							insertPs.setNull(5, java.sql.Types.INTEGER);
						if (checkerAction != null)
							insertPs.setString(6, checkerAction);
						else
							insertPs.setNull(6, java.sql.Types.VARCHAR);
						if (finalRemarks != null)
							insertPs.setString(7, finalRemarks);
						else
							insertPs.setNull(7, java.sql.Types.VARCHAR);
