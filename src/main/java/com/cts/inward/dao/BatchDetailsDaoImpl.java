				...
				try (ResultSet rs = statement.executeQuery()) {
					...
					while (rs.next()) {
						...
						if ("ACCOUNT_NUMBER".equals(fieldName)) {
							...
							details.put("newDrawerName", newValue);
						}
					}
				}
			}
	...
	public void saveCheckerDecision(String chequeNumber, String status, String rejectionReasonCode,
		...
		try (java.sql.Connection connection = dataSource.getConnection();
			...
			if (rejectionReasonCode == null) {
				...
				ps.setString(2, rejectionReasonCode);
			}

			if (returnReasonCode == null) {
				ps.setNull(3, java.sql.Types.VARCHAR);
			} else {
				ps.setString(3, returnReasonCode);
			}

			if (checkerId == null) {
				ps.setNull(4, java.sql.Types.BIGINT);
			} else {
				ps.setLong(4, checkerId);
			}
			...
			if (remarks == null || remarks.trim().isEmpty()) {
				...
				ps.setString(6, remarks.trim());
			}

			ps.setString(7, chequeNumber);
			ps.setLong(8, checkerId);

			int updatedRows = ps.executeUpdate();

			if (updatedRows == 0) {
				throw new RuntimeException(
						"No SENT_TO_CHECKER record found for cheque " + chequeNumber + " and checker " + checkerId);
			}
