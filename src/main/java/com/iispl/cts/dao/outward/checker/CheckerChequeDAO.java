...
public class CheckerChequeDAO {
	 ...
	 *
	 * 1. batch_number 2. original checker_id 3. checker_action = SEND_BACK 4.
	 * cheque_status = RE_VERIFIED
	 *
	 * outward_batch.batch_status is deliberately NOT used.
	 *
	 * This returns only the cheques that were sent back by the current/original
	 * Checker and subsequently corrected by Maker.
	 */
	 ...
	 * PROCESSING ============================================================
	 *
	 * Moves a Maker re-verified cheque back into Checker processing.
	 *
	 ...
	 *
	 * RE_VERIFIED ↓ Checker re-verifies
	 *
	 * ACCEPT / REJECT ↓ Final
	 *
	 * SEND_BACK ↓ ON_HOLD
	...
	public boolean saveCheckerDecision(String batchNumber, String chequeNumber, long checkerId, String checkerAction,
		 ...
		 *
		 * If previous action is SEND_BACK, this cheque is being processed again after
		 * Maker correction.
		 */
		 ...
		 *
		 * SENT_BACK_TO_MAKER = waiting for Maker
		 *
		 * RE_VERIFIED = corrected and waiting for Checker
		 *
		 * CHECKER_PROCESSING = currently being re-verified
		 */
		...
		try (Connection connection = CTSStaticData.getConnection()) {
			...
			try {
				...
				/*
				 * If the previous action was SEND_BACK, this is a re-verification decision.
				 */
				...
				if (reVerification) {
					...
					if (pendingMakerCheques > 0) {
						...
						/*
						 * Maker has corrected some/all returned cheques, but at least one is still
						 * waiting for Checker.
						 */
				 ...
				 *
				 * But the batch MUST remain CHECKER_PROCESSING until every cheque has been
				 * processed.
				 */
